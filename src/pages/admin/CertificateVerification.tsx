...
import { Award, CheckCircle2, XCircle, Search, User } from 'lucide-react'
...
export default function AdminCertificateVerification() {
  ...
  const handleAction = async () => {
    ...
    if (actionType === 'approve') {
      ...
      if (!error) {
        await supabase.from('certificates').update({ verified: true }).eq('id', actionTarget.reference_id)
      }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight">
          <Award className="size-6 text-amber-500" /> Verifikasi Sertifikat
        </h1>
        <p className="text-muted-foreground">Review dan verifikasi pengajuan sertifikat mahasiswa</p>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Cari nama sertifikat atau mahasiswa..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          <Award className="mx-auto mb-3 size-10 opacity-30" />
          <p className="font-medium">Tidak ada pengajuan sertifikat</p>
        </div>
      ) : (
                ...
                <CardContent className="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center">
                  ...
                  {sub.status === 'waiting' && (
                    <div className="flex gap-2 shrink-0">
                      <Button size="sm" onClick={() => { setActionTarget(sub); setActionType('approve'); setPointsAwarded('20') }} className="gap-1">
      ...
      <Dialog open={!!actionTarget} onOpenChange={(o) => !o && setActionTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{actionType === 'approve' ? 'Setujui Pengajuan Sertifikat' : 'Tolak Pengajuan Sertifikat'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Sertifikat: <strong>{actionTarget?.reference_name}</strong> dari <strong>{actionTarget?.profiles?.full_name}</strong>
            </p>
            {actionType === 'approve' ? (
