...
import { FolderCheck, CheckCircle2, XCircle, Search, User } from 'lucide-react'
...
export default function AdminPortfolioVerification() {
  ...
  const [pointsAwarded, setPointsAwarded] = useState('30')
  ...
  const handleAction = async () => {
    ...
    if (actionType === 'approve') {
      ...
      if (!error) {
        await supabase.from('portfolios').update({ verified: true }).eq('id', actionTarget.reference_id)
      }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight">
          <FolderCheck className="size-6 text-purple-500" /> Verifikasi Portofolio
        </h1>
        <p className="text-muted-foreground">Review dan verifikasi pengajuan portofolio mahasiswa</p>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Cari nama proyek atau mahasiswa..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          <FolderCheck className="mx-auto mb-3 size-10 opacity-30" />
          <p className="font-medium">Tidak ada pengajuan portofolio</p>
        </div>
      ) : (
                ...
                <CardContent className="flex flex-col gap-3 pt-4 sm:flex-row sm:items-center">
                  ...
                  {sub.status === 'waiting' && (
                    <div className="flex gap-2 shrink-0">
                      <Button size="sm" onClick={() => { setActionTarget(sub); setActionType('approve'); setPointsAwarded('30') }} className="gap-1">
                        <CheckCircle2 className="size-3.5" /> Setujui
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => { setActionTarget(sub); setActionType('reject'); setRejectionReason('') }} className="gap-1">
      ...
      <Dialog open={!!actionTarget} onOpenChange={(o) => !o && setActionTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{actionType === 'approve' ? 'Setujui Pengajuan Portofolio' : 'Tolak Pengajuan Portofolio'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Portofolio: <strong>{actionTarget?.reference_name}</strong> dari <strong>{actionTarget?.profiles?.full_name}</strong>
            </p>
            {actionType === 'approve' ? (
              <div className="space-y-2">
                <Label>Poin Diberikan</Label>
                <Input type="number" value={pointsAwarded} onChange={(e) => setPointsAwarded(e.target.value)} min={0} max={200} />
              </div>
            ) : (
              <div className="space-y-2">
